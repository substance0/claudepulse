import { formatLocalIso } from "./time-format.js";
/**
...
class PerformanceTracker {
  ...
  getSnapshot() {
    return {
      ...
      timestamp: formatLocalIso(new Date()),
    };
  }
...
export class Logger {
  constructor(options = {}) {
    ...
    this.format = options.format || process.env.LOG_FORMAT || "inline";
    this.enableJson = this.format === "json";
    // Color control: honor NO_COLOR/FORCE_COLOR, then fallback to TTY detection
    const envForceColor = (process.env.FORCE_COLOR || "").toString().toLowerCase();
    const envNoColor = (process.env.NO_COLOR || "").toString().toLowerCase();
    const forceColor = envForceColor && envForceColor !== "0" && envForceColor !== "false";
    const noColor = envNoColor && envNoColor !== "0" && envNoColor !== "false";
    let enableColors =
      options.enableColors !== undefined
        ? options.enableColors
        : this.format === "inline" && process.stdout.isTTY;
  ...
  _formatDataForInline(data, category) {
    ...
    switch (category.toLowerCase()) {
      ...
      case 'client':
        ...
        if (data.environmentVariables && data.environmentVariables.claudeCliPathOverride && data.environmentVariables.claudeCliPathOverride !== 'not set') parts.push('override=env');
        return parts.join(', ');

      case 'update':
        const sessions = `${data.activeSessions}/${data.totalSessions} sessions`;
        const current = data.currentSessionId ? `current=${data.currentSessionId.substring(0, 8)}` : '';
        const expiry = data.windowExpiry ? `expires=${formatLocalIso(new Date(data.windowExpiry))}` : '';
      ...
      case 'scheduler':
        ...
        if (data.intervalMs) {
          const nextRun = formatLocalIso(new Date(data.nextRunTime));
      ...
      case 'message':
        {
          ...
          // request properties
          const len = (typeof data.promptLength === 'number') ? data.promptLength : (data.prompt ? String(data.prompt).length : undefined);
          if (typeof len === 'number') parts.push(`prompt_len=${len}`);
          if (data.prompt) parts.push(`prompt="${String(data.prompt).slice(0,120)}"`);
          // response properties
          if (data.sessionId) parts.push(`session=${String(data.sessionId).substring(0,8)}`);
          if (data.totalCostUsd !== undefined) parts.push(`cost=$${data.totalCostUsd}`);
          if (data.durationMs !== undefined) parts.push(`duration=${data.durationMs}ms`);
          if (data.resultLength !== undefined) parts.push(`resp_len=${data.resultLength}`);
          if (data.result) {
            const preview = String(data.result).replace(/\s+/g, ' ').slice(0, 200);
            parts.push(`result="${preview}${data.result.length > 200 ? '…' : ''}"`);
          }
          return parts.join(', ');
        }
      ...
      case 'auth':
        if (data.connectionTest && data.connectionTest.success !== undefined) {
          return data.connectionTest.success ? 'connection_test_passed' : 'connection_test_failed';
        }
        if (typeof data.authenticated === 'boolean') return data.authenticated ? 'authenticated' : 'not_authenticated';
        return '';
      ...
      case 'expiry':
        if (data.cycleStartUTC || data.hourBoundaryExpiry || data.firstMessageUTC) {
          const parts = [];
          if (data.cycleStartUTC) parts.push(`start=${formatLocalIso(new Date(data.cycleStartUTC))}`);
          if (data.hourBoundaryExpiry) parts.push(`expiry=${formatLocalIso(new Date(data.hourBoundaryExpiry))}`);
          if (data.firstMessageUTC) parts.push(`first=${formatLocalIso(new Date(data.firstMessageUTC))}`);
          return parts.join(', ');
        }
      ...
      case 'schedule':
        if (data.sessionExpiry) {
          const expiryIso = formatLocalIso(new Date(data.sessionExpiry));
          return `session_expires=${expiryIso}`;
        }
        if (data.planned || data.strategy || data.intervalMs !== undefined) {
          const parts = [];
          if (data.planned) parts.push(`planned=${formatLocalIso(new Date(data.planned))}`);
          if (data.strategy) parts.push(`strategy=${data.strategy}`);
          if (data.intervalMs !== undefined) parts.push(`intervalMs=${data.intervalMs}`);
          return parts.join(', ');
        }
        return '';
      ...
      case 'ratelimit':
        {
          const parts = [];
          if (data.resetTimeRaw) parts.push(`raw="${String(data.resetTimeRaw).slice(0,80)}${String(data.resetTimeRaw).length>80?'…':''}"`);
          if (data.method) parts.push(`method=${data.method}`);
          const base = data.localIso ? new Date(data.localIso) : new Date(data.resetAt || data.iso || Date.now());
          parts.push(`at=${formatLocalIso(base)}`);
          return parts.join(', ');
        }
      ...
      case 'dry-run':
        if (data.optimalSchedule) {
          const optimal = formatLocalIso(new Date(data.optimalSchedule));
      ...
      default:
        ...
        ['error', 'message', 'status', 'duration', 'count', 'id', 'name', 'path'].forEach(prop => {
          if (data[prop] !== undefined) {
            meaningful.push(`${prop}=${data[prop]}`);
          }
        });
  ...
  _formatLogEntry(level, category, message, data = null, metadata = {}) {
    ...
    if (this.enableJson) {
      ...
      // Plain text format with optional colors
   ...
   * Log error message
   */
  error(category, message, data = null, metadata = {}) {
  ...
  /**
   * Log Claude CLI command execution
   */
  logCli(command, success, exitCode, duration = null, data = null) {
    const message = `CLI command '${command}' ${success ? "succeeded" : "failed"} (exit ${exitCode})`;

    this.performance.incrementCounter(success ? "cli.success" : "cli.failure");
    ...
    if (duration) {
      metadata.cliDurationMs = duration.ms;
    }

    this._log(LOG_LEVELS.DEBUG, "cli", message, data, metadata);
  }
